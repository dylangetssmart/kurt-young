...
--where
--	ioci.source_ref like '%user_case_data.employer_name%'

/* ------------------------------------------------------------------------------
Create employment records from user_case_data.employer_name
*/
insert into [dbo].[sma_TRN_Employment]
	...
	select distinct
		...
		ucd.AWW						   as [empnAverageWeeklyWage],		--Average weekly wage (AWW)
	...
	join IndvOrgContacts_Indexed ioci
		on ioci.Name = ucd.Employer_Name
			and ioci.source_ref = 'user_case_data.employer_name'
	where
