	...
	select
		...
		(
			select
				sbrnSubRoleId
			from sma_MST_SubRole
			where sbrsDscrptn = 'Lead Attorney'
				and sbrnRoleID = 10
		)				as [cssnRoleID],
		null			as [csssComments],
		null			as cssdFromDate,
	...
	select
		...
		(
			select
				sbrnSubRoleId
			from sma_MST_SubRole
			where sbrsDscrptn = 'Appeals Paralegal'
				and sbrnRoleID = 10
		)				as [cssnRoleID],
		null			as [csssComments],
		null			as cssdFromDate,
