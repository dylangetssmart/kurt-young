...
insert into [sma_TRN_InsuranceCoverage]
	...
	select distinct
		...
		(
			...
			where defnCaseID = cas.casnCaseID
				and defbIsPrimary = 1
		...
		i.id									as [saga_char],
		null									as [source_db],
		iocins.saga_char						as [source_ref]		-- litify account.id of insurance company from RoleMap
	--select * --iocins.CID, iocadj.Id
	...
	left join vw_litifyRoleMapID iocins
		on iocins.ID = i.litify_pm__Insurance_Company__c
			and iocins.CTG = 2
			and iocins.litify_pm__Matter__c = i.litify_pm__Matter__c
	...
	--	'HMO')
	where
		i.litify_pm__Coverages__c is null
		or i.litify_pm__Coverages__c in ('Liability;Med Pay',
		'BI;Med Pay',
		'NULL',
		'Liability;BI',
		'Collision;UM;PIP;Med Pay',
		'Collision;No UM',
		'Collision',
		'BI;No UM',
		'BI',
...
--and i.litify_pm__Matter__c = 'a0L8Z00000gCdZaUAK'
