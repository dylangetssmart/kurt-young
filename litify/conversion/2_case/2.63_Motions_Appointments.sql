...
--select
--	*
--from sma_TRN_Motions stm where stm.mtnnCaseID = 545

--select
--	*
--from ShinerSA..sma_TRN_CalendarAppointments stca
--where
--	stca.CaseID = 545
...
--select
--	*
--from sma_MST_AppointmentType smat
--select
--	*
--from sma_MST_ActivityType smat
--select
--	*
--from sma_MST_AppointmentStatus mas
...
alter table sma_TRN_Motions disable trigger all
go
...
join sma_TRN_CalendarAppointments cal
	...
	and m.[source_ref] = 'Motion__c'					 

alter table sma_TRN_Motions enable trigger all
go