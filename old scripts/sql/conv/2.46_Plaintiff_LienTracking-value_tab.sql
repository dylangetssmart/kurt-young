...
/*
alter table [SANeedlesKMY].[dbo].[sma_TRN_Lienors] disable trigger all
delete from [SANeedlesKMY].[dbo].[sma_TRN_Lienors] 
DBCC CHECKIDENT ('[SANeedlesKMY].[dbo].[sma_TRN_Lienors]', RESEED, 0);
alter table [SANeedlesKMY].[dbo].[sma_TRN_Lienors] enable trigger all

alter table [SANeedlesKMY].[dbo].[sma_TRN_LienDetails] disable trigger all
delete from [SANeedlesKMY].[dbo].[sma_TRN_LienDetails] 
DBCC CHECKIDENT ('[SANeedlesKMY].[dbo].[sma_TRN_LienDetails]', RESEED, 0);
alter table [SANeedlesKMY].[dbo].[sma_TRN_LienDetails] enable trigger all
...
select count(*) from [SANeedlesKMY].[dbo].[sma_TRN_Lienors]

select * from value_tab_Liencheckbox_Helper 

select * from [NeedlesKMY].[dbo].[value_payment] where value_id=65990
...
CREATE NONCLUSTERED INDEX IX_NonClustered_Index_value_tab_Liencheckbox_Helper_value_id ON [SANeedlesKMY].[dbo].[value_tab_Liencheckbox_Helper] (value_id);   
GO
...
select VP1.value_id
from [NeedlesKMY].[dbo].[value_payment] VP1 
    left join (
                select distinct value_id
                from [NeedlesKMY].[dbo].[value_payment]
                where lien='Y'
                ) VP2 
...
---(0)---
insert into [SANeedlesKMY].[dbo].[sma_MST_LienType]
(
    [lntsCode]
    ,[lntsDscrptn]
)
(
    ...
    from [NeedlesKMY].[dbo].[value] V
    inner join [NeedlesKMY].[dbo].[value_code] VC
        on VC.code = V.code 
    where isnull(V.code,'') in (SELECT code FROM #LienValueCodes)
)
except
select [lntsCode],[lntsDscrptn] from [SANeedlesKMY].[dbo].[sma_MST_LienType] 
GO
...
CREATE NONCLUSTERED INDEX IX_NonClustered_Index_value_tab_Lien_Helper_case_id ON [SANeedlesKMY].[dbo].[value_tab_Lien_Helper] (case_id);   
CREATE NONCLUSTERED INDEX IX_NonClustered_Index_value_tab_Lien_Helper_value_id ON [SANeedlesKMY].[dbo].[value_tab_Lien_Helper] (value_id);   
CREATE NONCLUSTERED INDEX IX_NonClustered_Index_value_tab_Lien_Helper_ProviderNameId ON [SANeedlesKMY].[dbo].[value_tab_Lien_Helper] (ProviderNameId);   
GO
...
select
    ...
    null			    as PlaintiffID,
    null			    as Paid
from [NeedlesKMY].[dbo].[value_Indexed] V
inner join [SANeedlesKMY].[dbo].[sma_TRN_cases] CAS
    on CAS.NeedlesCasenum= V.case_id
    -- on CAS.cassCaseNumber = V.case_id
inner join [SANeedlesKMY].[dbo].[IndvOrgContacts_Indexed] IOC
...
select 
    V.case_id		    as cid,	
    V.value_id		    as vid,
    convert(varchar,((select sum(payment_amount) from [NeedlesKMY].[dbo].[value_payment] where value_id=V.value_id))) as Paid,
    T.plnnPlaintiffID
...
inner join [SANeedlesKMY].[dbo].[sma_TRN_cases] CAS 
    ...
    inner join [SANeedlesKMY].[dbo].[sma_TRN_Plaintiff] T
...
select 
    ...
    ,(
        ...
        from [SANeedlesKMY].[dbo].[sma_TRN_Plaintiff]
...
from [NeedlesKMY].[dbo].[value_Indexed] V
    ...
    inner join [SANeedlesKMY].[dbo].[IndvOrgContacts_Indexed] IOC
        on IOC.SAGA = V.party_id
    inner join [SANeedlesKMY].[dbo].[sma_TRN_Defendants] D
...
GO
---(1)---
insert into [SANeedlesKMY].[dbo].[sma_TRN_Lienors]
(
    [lnrnCaseID],
    [lnrnLienorTypeID],
  ...
  select 
    ...
    ( select top 1 lntnLienTypeID FROM [SANeedlesKMY].[dbo].[sma_MST_LienType] where lntsDscrptn=
	   (select [description] FROM [NeedlesKMY].[dbo].[value_code] where [code]=V.code)) 
						  as [lnrnLienorTypeID],				   
    ...
    V.value_id				  as [saga]
from [NeedlesKMY].[dbo].[value_Indexed] V
    inner join [SANeedlesKMY].[dbo].[value_tab_Lien_Helper] MAP
        on MAP.case_id = V.case_id
        and MAP.value_id = V.value_id

---(2)---
insert into [SANeedlesKMY].[dbo].[sma_TRN_LienDetails]
(
	lndnLienorID,
	lndnLienTypeID,
...
from [SANeedlesKMY].[dbo].[sma_TRN_Lienors]
