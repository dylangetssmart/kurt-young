...
FROM ( 
    ...
    JOIN sma_TRN_Cases cas ON cas.NeedlesCasenum = CONVERT(VARCHAR, ud.casenum)
...
FROM ( 
    ...
    JOIN sma_TRN_Cases cas ON cas.NeedlesCasenum = CONVERT(VARCHAR, ud.case_id)
...
/* ####################################
ds 2024-08-14
--1.3 - Insert alt_case_num into CaseUDF
*/
...
FROM ( 
    SELECT
		...
		,case
			when c.alt_case_num is null
				then 'null'
			when LTRIM(RTRIM(c.alt_case_num)) = ''
				then 'blank'
			else convert(varchar(max), [alt_case_num])
		end as [Alt Case #]
    FROM NeedlesKMY..cases c
    JOIN sma_TRN_Cases cas ON cas.NeedlesCasenum = CONVERT(VARCHAR, c.casenum)
    -- JOIN sma_TRN_Cases cas ON cas.cassCaseNumber = CONVERT(VARCHAR, ud.case_id)
) pv
UNPIVOT (FieldVal FOR FieldTitle IN (
	[Alt Case #]
