...
FROM [NeedlesKMY].[dbo].[value_Indexed] V
JOIN [sma_TRN_cases] CAS on CAS.NeedlesCasenum= V.case_id
...
select 
    V.case_id		    as cid,	
    V.value_id		    as vid,
    ( select plnnPlaintiffID from [SANeedlesKMY].[dbo].[sma_TRN_Plaintiff] where plnnCaseID=CAS.casnCaseID and plnbIsPrimary=1) as plnnPlaintiffID 
    into value_tab_Multi_Party_Helper_Temp   
from [NeedlesKMY].[dbo].[value_Indexed] V
JOIN [sma_TRN_cases] CAS on CAS.NeedlesCasenum= V.case_id
-- JOIN [sma_TRN_cases] CAS on CAS.cassCaseNumber = V.case_id
...
JOIN [NeedlesKMY].[dbo].[value_payment] VP
