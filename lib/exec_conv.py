...
SQL_SCRIPTS_DIR = os.path.join(BASE_DIR, '../sql-scripts/fromKMY/')
...
def exec_conv(options):
    ...
    sequence = int(options.get('sequence'))
    ...
    try:
        ...
        if not files:
            ...
            with Progress(
                ...
                task = progress.add_task(f"[yellow]Executing SQL Sequence {sequence}", total=len(files))
                for file in files:
